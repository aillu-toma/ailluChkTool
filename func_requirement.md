...
# 日次データを取り扱うカラムのフォーマットについて
カラム毎に異なるフォーマットを採用している
...
前提として、勤務区分 = 空白の場合は処理スキップしチェックを行わない
    ...
    勤務区分 = EFS01の場合、
        ...
        届出区分 = 85　のデータが存在する場合はエラーを出力
        存在しない場合はチェック１を行う

    勤務区分がZから始まる場合、
        トランザクションデータの項目：届出区分を確認
        届出区分 = 85　のデータが存在しない場合はエラーを出力
